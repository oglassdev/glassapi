...
object EntityUtil {
    ...
    fun setInvulnerable(entity: Entity, value: Boolean) {
        NBT.modify(entity) { nbt: ReadWriteNBT -> nbt.setByte("Invulnerable",(if (value) 1 else 0).toByte()) }
    }
    fun setAI(entity: Entity, value: Boolean) {
        NBT.modify(entity) { nbt: ReadWriteNBT -> nbt.setByte("NoAI",(if (value) 0 else 1).toByte()) }
    }
    fun setLocation(stand: LivingEntity, x: Double, y: Double, z: Double, yaw: Float, pitch: Float) {
        try {
